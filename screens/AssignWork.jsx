...
import { View, Text, StyleSheet, FlatList, Image, TouchableOpacity, Picker, Platform } from 'react-native';
    ...
    <View style={{ flex: 1, backgroundColor: '#f1f5f9', padding: 12 }}>
      ...
      {acceptedReports.length === 0 ? (
        ...
        <>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                ...
                style={[styles.reportRow, selectedReportId === item.id ? { borderColor: '#4e9bde', borderWidth: 2 } : null]}
              >
          ...
          {Platform.OS === 'android' ? (
            <View style={styles.pickerWrap}>
              <Picker selectedValue={selectedContractor} onValueChange={(val) => setSelectedContractor(val)}>
                {CONTRACTORS.map(c => <Picker.Item key={c.name} label={`${c.name} (${c.specialization})`} value={c.name} />)}
              </Picker>
            </View>
          ) : (
            <View style={styles.pickerWrap}>
              {CONTRACTORS.map(c => (
                <TouchableOpacity key={c.name} onPress={() => setSelectedContractor(c.name)} style={[styles.contractorBtn, selectedContractor === c.name ? styles.contractorBtnActive : null]}>
                  <Text style={selectedContractor === c.name ? { color: '#fff' } : {}}>{c.name} ({c.specialization})</Text>
                </TouchableOpacity>
              ))}
            </View>
          )}
          ...
          <Picker selectedValue={selectedType} onValueChange={setSelectedType} style={styles.pickerWrap}>
